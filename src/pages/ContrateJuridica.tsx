...
const CollectedDataFooter = ({ geolocation }: { geolocation?: string }) => {
  ...
  return (
    <div className="md:col-span-2 mt-2">
      <small className="block text-medium-gray text-[9px] leading-relaxed">
...
const ContrateJuridica = () => {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (found) {
        ...
        setCouponAlert({ type: "success", text: describeCouponBenefit(found) });
      } else {
  ...
  const validateAll = (): boolean => {
    ...
    if (form.razaoSocial.trim().split(/\s+/).length < 2)
      return fail("razaoSocial", "A razão social deve conter ao menos duas palavras.");
    ...
    // Document
    if (!docUpload.doc1) {
      showAlert("danger", "Anexe o Contrato Social.");
      return false;
    }
    ...
    if (!termsAccepted) {
      showAlert("danger", "Você deve aceitar o contrato e os termos de uso.");
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      updateAndFlush("validate", { status: "done", detail: "Dados validados." });

      // Metadata
      updateAndFlush("metadata", { status: "running" });

      const recaptchaToken = await getToken("contrate_pj");
      let ipAddress = "";
      try {
      ...
      updateAndFlush("docs", { status: "done", detail: docUpload.doc1 ? "1 documento enviado." : "Nenhum documento." });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Inscrição Estadual{form.ieIsento ? "" : "*"}
                    </label>
                    <input
                      type="text"
                      placeholder={form.ieIsento ? "Isento" : "Inscrição Estadual"}
                      inputMode="numeric"
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      type="text"
                      placeholder="Ex.: Sócio, Gerente, Cônjuge..."
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Nome completo"
                      value={form.financialName}
                      onChange={(e) => setField("financialName", e.target.value)}
                      className={inputCls("financialName")}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Telefone*</label>
                    <input
                      ...
                      placeholder="(11) 90000-0000"
                      maxLength={15}
                      value={form.financialPhone}
                      onChange={(e) => setField("financialPhone", formatPhoneBR(onlyDigits(e.target.value)))}
                      className={inputCls("financialPhone")}
                    />
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) => setField("financialEmail", e.target.value)}
                      className={inputCls("financialEmail")}
                    />
                  </div>
                  ...
                  <div className="md:col-span-2">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <p className="text-sm text-blue-700">
                        <strong>Frota (múltiplos veículos):</strong> Caso possua mais de um veículo, preencha os dados
                        de apenas um como referência. Nosso atendimento irá solicitar as informações dos demais veículos
